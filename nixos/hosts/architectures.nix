...
let
	cfg = common.core.hardware-manifest.architectures;
in

{
		...
		gpu = 
		{
			...
			let
				add-arch = (
					...
					{
						inherit year;
						driver = if driver-name == null then null else config.boot.kernelPackages.nvidiaPackages.${driver-name};
					}
				);
			in
			{
	...
	options.common.core.hardware-manifest.architectures =
	let
		year-opt = jc-bin.mkIntOptionRO "The year the architecture was released";
	in
	{
		cpu = 
		{
		...
		gpu = 
		{
			...
			nvidia = jc-lib.mkSubmodOptionRO "Nvidia gpu architecures" (
				...
				{
					inherit year-opt;
					greater-equal-than-turing = lib.mkOption {
						description = "If this architecture comes after the turing serie";
						type = lib.types.bool;
						default = cfg.nvidia."${name}".year >= cfg.nvidia.turing.year;
						readonly = true;
					};
					driver-pkg = lib.mkOption {
